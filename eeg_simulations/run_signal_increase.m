...
for k=1:ntrials
  ...
  % trial-by-trial jitter of the noise quench. This parameter only does
  % something when "var_decrease" is different from "noise_level" (see
  % Fig1_var_decrease.m)
  ...
  a_g1_imp = normrnd(2,0.2);
  a_g2_imp = normrnd(3.5,0.2);
  b_g1_imp = normrnd(40,0.5);
  b_g2_imp = normrnd(50,0.5);
  imp_dur = t0idx_imp_j:size(t,2);
  
  g1_i = gamwaveform(1:size(imp_dur,2),a_g1_imp,b_g1_imp);
  g2_i = gamwaveform(1:size(imp_dur,2),a_g2_imp,b_g2_imp);
  
  g1_imp = [zeros(1,t0idx_imp_j-1) g1_i];
  g2_imp = [zeros(1,t0idx_imp_j-1) g2_i];
  slow1_imp = g1_imp.*0.3;
  slow2_imp = g2_imp.*0.12;
  ...
  erp(k,1,:) = slow1_imp + slow1 + noise1;
  erp(k,2,:) = slow2_imp + slow2 + noise2; 
end
...
subplot(211);
imagesc(t,1:size(wins,2)-1,squeeze(mean(dec,2)),[0 0.2]);colorbar;colormap hot;
