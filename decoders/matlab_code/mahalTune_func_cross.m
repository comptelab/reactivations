function  [cos_amp, d_tune] = mahalTune_func(data,data_test,theta,theta_test,angspace,bin_width)
...
for trl=1:size(data_test,1)
    ...
    for b=1:length(angspace)
        ...
        m(b,:,:)=mean(trn_dat(abs(angle(exp(1i*trn_angle)./exp(1i*(theta_test(trl)-angspace(b)))))<bin_width,:,:),1);
    end
    ...
    for ti=1:size(data,3)
        if ~isnan(trn_dat(:,:,ti))
            % the covariance matrix is computed for each time-point using
            % training and test set then average them (?)
            sigma1 = covdiag(trn_dat(:,:,ti));
            sigma2 = covdiag(data_test(:,:,ti));
            
            sigma = sigma1/2 + sigma2/2
          
            % calculates the distances between the trial and all angle bins
            d_tune(trl,:,ti) = pdist2(squeeze(m(:,:,ti)), squeeze(data_test(trl,:,ti)),'mahalanobis',sigma);
